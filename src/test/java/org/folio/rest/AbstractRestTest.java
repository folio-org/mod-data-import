...
public abstract class AbstractRestTest {
  ...
  private JsonObject configurationStoragePath = new JsonObject()
    .put("totalRecords", 1)
    .put(
      "configs",
      new JsonArray()
  ...
  private JsonObject configurationStorageType = new JsonObject()
    .put("totalRecords", 1)
    .put(
      "configs",
      new JsonArray()
  ...
  private JsonObject configurationCleanupTime = new JsonObject()
    .put("totalRecords", 1)
    .put(
      "configs",
      new JsonArray()
  ...
  public static void setUpClass(final TestContext context) throws Exception {
    ...
    switch (useExternalDatabase) {
      ...
      default:
        String message =
          "No understood database choice made." +
    ...
    vertx.deployVerticle(
      ...
      res -> {
        try {
          ...
          tenantAttributes.setModuleTo(
            ModuleName.getModuleName() + TEST_MODULE_VERSION
          );
              ...
              if (res2.result().statusCode() == 204) {
                ...
                tenantClient.getTenantByOperationId(
                  ...
                  context.asyncAssertSuccess(res3 -> {
                    ...
                    context.assertTrue(
                      res3.bodyAsJson(TenantJob.class).getComplete()
                    );
                    String error = res3.bodyAsJson(TenantJob.class).getError();
                    if (error != null) {
                      context.fail(
                        "Failed to make post tenant. Received error: " +
                        res3.bodyAsString()
                      );
                    }
                  })
                ...
                context.fail(
                  "Failed to make post tenant. Received non-2xx status code: " +
  ...
  public void setUp(TestContext context) throws IOException {
    ...
    WireMock.stubFor(
      get(GET_USER_URL + okapiUserIdHeader)
    ...
    WireMock.stubFor(
      get(urlPathEqualTo("/configurations/entries"))
        .withQueryParam(
          "query",
          equalTo(
            "module==DATA_IMPORT AND ( code==\"data.import.storage.path\")"
          )
        )
    ...
    WireMock.stubFor(
      get(urlPathEqualTo("/configurations/entries"))
        .withQueryParam(
          "query",
          equalTo(
            "module==DATA_IMPORT AND ( code==\"data.import.storage.type\")"
          )
        )
        .withQueryParam("offset", equalTo("0"))
        .withQueryParam("limit", equalTo("3"))
    ...
    WireMock.stubFor(
      get(urlPathEqualTo("/configurations/entries"))
        .withQueryParam(
          "query",
          equalTo(
            "module==DATA_IMPORT AND ( code==\"data.import.cleanup.time\")"
          )
        )
        .withQueryParam("offset", equalTo("0"))
        .withQueryParam("limit", equalTo("3"))
    ...
    WireMock.stubFor(
      post("/change-manager/jobExecutions")
        ...
        .willReturn(
          created()
            .withBody(
              JsonObject.mapFrom(jobExecutionCreateSingleFile).toString()
            )
        )
    );
    WireMock.stubFor(
      post("/change-manager/jobExecutions")
        ...
        .willReturn(
          created()
            .withBody(
              JsonObject.mapFrom(jobExecutionCreateMultipleFiles).toString()
            )
        )
    );
    WireMock.stubFor(
      put(
        new UrlPathPattern(
          new RegexPattern("/change-manager/jobExecutions/.*"),
          true
        )
      )
        .willReturn(ok())
    ...
    WireMock.stubFor(
      get(
        new UrlPathPattern(
          new RegexPattern("/change-manager/jobExecutions/.{36}"),
          true
        )
      )
    ...
    WireMock.stubFor(
      get(
        new UrlPathPattern(
          new RegexPattern("/change-manager/jobExecutions/.{36}/children"),
          true
        )
      )
        .willReturn(
          okJson(JsonObject.mapFrom(childrenJobExecutions).toString())
        )
    );
  }
  ...
  protected void clearTable(TestContext context) {
    ...
    PostgresClient
      .getInstance(vertx, TENANT_ID)
      .delete(
        FILE_EXTENSIONS_TABLE,
        new Criterion(),
        context.asyncAssertSuccess(event1 ->
          PostgresClient
            .getInstance(vertx, TENANT_ID)
            .delete(
              UPLOAD_DEFINITIONS_TABLE,
              new Criterion(),
              context.asyncAssertSuccess(event2 ->
                PostgresClient
                  .getInstance(vertx)
                  .execute(
                    "DELETE FROM data_import_global.queue_items;",
  ...
  private static WireMockServer mockTenantUserCalls(String realUrl) {
    WireMockServer tenantMockServer = new WireMockServer(
    ...
    log.info(
      "Started mocking system user creation API calls on port {} in preparation for /_/tenant",
      tenantMockServer.port()
    );
    ...
    tenantMockServer.stubFor(
      post("/_/tenant").willReturn(aResponse().proxiedFrom(realUrl))
    );
    ...
    tenantMockServer.stubFor(
      get(urlPathEqualTo("/users"))
        .withQueryParam(
          "query",
          equalTo("username=\"data-import-system-user\"")
        )
        .willReturn(
          okJson(
            JsonObject
              .of(
                "users",
                new JsonArray().add(User.builder().id("system-user-id").build())
              )
              .toString()
          )
        )
    ...
    tenantMockServer.stubFor(
      post(urlPathEqualTo("/authn/login"))
        .willReturn(okJson(JsonObject.of("okapiToken", "token").toString()))
    );
