...
public class QueueItemAgeRanker implements QueueItemRanker {
  ...
  private final int scoreAgeNewest;
  private final int scoreAgeOldest;
  private final int scoreAgeExtremeThresholdMinutes;
  private final int scoreAgeExtremeValue;

  @Autowired
  public QueueItemAgeRanker(@Value("${SCORE_AGE_NEWEST:0}") int scoreAgeNewest,
                            @Value("${SCORE_AGE_OLDEST:50}") int scoreAgeOldest,
                            @Value("${SCORE_AGE_EXTREME_THRESHOLD_MINUTES:480}") int scoreAgeExtremeThresholdMinutes,
