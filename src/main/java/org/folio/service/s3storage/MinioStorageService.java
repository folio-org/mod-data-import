...
public interface MinioStorageService {
   ...
   *   </ul>
   */
  Future<FileUploadInfo> getFileUploadFirstPartUrl(String uploadFileName, String tenantId);

  /**
   * Gets upload url and key for a file upload.
   *
   * @param key        - the key to access the file on S3 storage, as generated with
   *                   {@code #getFileFirstPartUploadUrl}
   * @param uploadId   - id of upload from previous calls to {@code #getFileFirstPartUploadUrl}
   * @param partNumber - the part number, starting at 2 (part number 1 will be
   *                   provided by {@code #getFileFirstPartUploadUrl})
   * @return FileUploadInfo
   *   <ul>
   *     <li>url: presigned URL to S3 storage</li>
   *     <li>key: key for access to file on S3 storage</li>
   ...
   *   </ul>
   */
  Future<FileUploadInfo> getFileUploadPartUrl(String key, @NotNull String uploadId, int partNumber);
   ...
   */
  Future<Void> completeMultipartFileUpload(String key, String uploadId, List<String> etags);
