...
public class ProcessS3APITest extends AbstractRestTest {
  ...
  public void testProcessingSuccess() throws IOException {
    ...
    WireMock.stubFor(WireMock.get(urlPathMatching("/change-manager/jobExecutions/[^/]*"))
      .willReturn(WireMock.okJson(Json.encode(new JobExecution()
    ...
    // waits for the change of the upload definition status to COMPLETED after processing
    // to keep the test continues running until asynchronous processing of the endpoint is finished
    // in order to avoid failure during tests run
    Awaitility.await().atMost(Duration.ofSeconds(3)).untilAsserted(() ->
      RestAssured.given()
        .spec(spec)
        .when()
        .get("/data-import/uploadDefinitions/{uploadDefinitionId}", uploadDefinition.getId())
        .then()
        .statusCode(HttpStatus.SC_OK)
